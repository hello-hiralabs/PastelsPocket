...
export function filterBar(hitung){
  ...
  return '<div class="mb-2.5">' +
    '<div class="amtbox flex items-center gap-2 px-2.5 py-1.5 rounded-2xl mb-2" ' +
      ...
      (filterActive()
        ? '<button data-act="q-clear" aria-label="Hapus filter" class="shrink-0 w-6 h-6 grid place-items-center rounded-full hand font-bold text-[13px]" style="background:#FFEAF0;color:#C9758A">×</button>'
        : '') +
    '</div>' +
